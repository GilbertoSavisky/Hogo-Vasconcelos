...
begin
    inherited;
    edt_num_doc.Text := dm_1.CDS_caixacx_num_doc.AsString;
    edt_descricao.Text := dm_1.CDS_caixacx_descricao.AsString;
    edt_valor.Text := FormatFloat(',#0.00', dm_1.CDS_caixacx_valor.AsCurrency);
    rg_tipo.ItemIndex := ifthen(dm_1.CDS_caixacx_tipo.AsString = 'C', 0, 1);
...
begin
    inherited;
    dm_1.CDS_caixa.Close;
    dm_1.CDS_caixa.CommandText := 'select * from caixa';
    dm_1.CDS_caixa.Open;
end;
...
begin
    ...
    if dataSource_tabela.State in [dsInsert] then
        dm_1.CDS_caixacx_id.AsInteger := get_id('cx_id', 'caixa');
    dm_1.CDS_caixacx_num_doc.AsString := Trim(edt_num_doc.Text);
    dm_1.CDS_caixacx_descricao.AsString := Trim(edt_descricao.Text);
    dm_1.CDS_caixacx_valor.AsCurrency := StringToFloat(edt_valor.Text);
    case rg_tipo.ItemIndex of
        0:
            dm_1.CDS_caixacx_tipo.AsString := 'C';
        1:
            dm_1.CDS_caixacx_tipo.AsString := 'D';
    end;
    dm_1.CDS_caixacx_data_cadastro.AsDateTime := Now;
    inherited;
