...
procedure Tfrm_cadastro_contas_receber2.act_deleteExecute(Sender: TObject);
begin
    if Application.MessageBox('Deseja realmente Cancelar o Documento?',
      'Pergunta', MB_YESNO) = mrYes then
    begin
        try
            dm_1.CDS_contas_receber.Edit;
            dm_1.cntRcb_status.AsString := 'C';
            dm_1.CDS_contas_receber.ApplyUpdates(0);

            Application.MessageBox('Documento Cancelado com Sucesso!',
              'Informação', 0 + 64);
...
procedure Tfrm_cadastro_contas_receber2.act_pesquisarExecute(Sender: TObject);
begin
    ...
    dm_1.CDS_contas_receber.CommandText := 'SELECT * FROM CONTAS_RECEBER';
    dm_1.CDS_contas_receber.Open;
    ...
    try
        ...
        while not CDS_parcelas.Eof do
            ...
            begin
                ...
                dm_1.cntRcb_vlr_compra.AsCurrency :=
                  StrToFloat(edt_vl_compra.Text);
                dm_1.cntRcb_dt_compra.AsDateTime :=
                  StrToDate(edt_dt_compra.Text);
                dm_1.cntRcb_dt_vencimento.AsDateTime :=
                  CDS_parcelasVencimento.AsDateTime;
                dm_1.cntRcb_status.AsString := 'A';
                dm_1.cntRcb_dt_cadastro.AsDateTime :=
                  StrToDate(edt_dt_compra.Text);
    ...
    case cbx_filtros.ItemIndex of
        0:
            dm_1.CDS_contas_receber.CommandText :=
              'select * from contas_receber where cntRcb_documento like ' +
              QuotedStr('%' + edt_pesquisar.Text + '%');
        1:
            dm_1.CDS_contas_receber.CommandText :=
              'select * from contas_receber where cntRcb_descricao like ' +
              QuotedStr('%' + edt_pesquisar.Text + '%');
    end;
    dm_1.CDS_contas_receber.Open;
end;
